...
# Be aware that beansd and beans-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in beansd.
#
...
# C locale.
#
# In contrast, beansd does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where beansd is guaranteed to be running
# with the classic locale ("C") whereas the locale of beans-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in beansd but
# "1,230000" in beans-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/beans-tx.cpp.*stoul"
    "src/beans-tx.cpp.*trim_right"
