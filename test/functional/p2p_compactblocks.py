...
# TestP2PConn: A peer we use to send messages to beansd, and store responses.
...
class CompactBlocksTest(BeansTestFramework):
    ...
    # This test actually causes beansd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that beansd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for beansd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        version = test_node.cmpct_version
        node = self.nodes[0]
        # beansd will not send blocktxn responses for blocks whose height is
