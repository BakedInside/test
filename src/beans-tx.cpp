...
static void SetupBeansTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " vertcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  vertcoin-tx [options] <hex-tx> [commands]  Update hex-encoded vertcoin transaction\n" +
            "or:     vertcoin-tx [options] -create [commands]   Create hex-encoded vertcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded beans transaction
