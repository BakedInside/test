...
#ifndef BEANS_POLICY_FEERATE_H
#define BEANS_POLICY_FEERATE_H
...
const std::string CURRENCY_UNIT = "BNS"; // One formatted unit
...
enum class FeeEstimateMode {
    ...
    BNS_KVB,      //!< Use BNS/kvB fee rate unit
...
public:
     ...
     *  where 1e5 is the ratio to convert from BNS/kvB to sat/vB.
     *
    ...
    std::string ToString(const FeeEstimateMode& fee_estimate_mode = FeeEstimateMode::BNS_KVB) const;

    SERIALIZE_METHODS(CFeeRate, obj) { READWRITE(obj.nSatoshisPerK); }
};

#endif //  BEANS_POLICY_FEERATE_H
