...
const char * const BEANS_CONF_FILENAME = "vertcoin.conf";
const char * const BEANS_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/beans/beans/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //beans-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", BEANS_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Vertcoin
    // macOS: ~/Library/Application Support/Vertcoin
    // Unix-like: ~/.vertcoin
#ifdef WIN32
    // Windows
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", BEANS_CONF_FILENAME);
