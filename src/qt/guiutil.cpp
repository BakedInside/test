...
#include <qt/beansaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new BeansAddressEntryValidator(parent));
    widget->setCheckValidator(new BeansAddressCheckValidator(parent));
}

bool parseBeansURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no beans: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BeansUnits::parse(BeansUnits::BNS, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseBeansURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBeansURI(uriInstance, out);
}

QString formatBeansURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BeansUnits::format(BeansUnits::BNS, info.amount, false, BeansUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBeansConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", BEANS_CONF_FILENAME));
    ...
    /* Open beans.conf with the associated application */
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a beans.desktop file to the autostart directory:
