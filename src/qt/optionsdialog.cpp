    ...
    mapper(nullptr)
{
    ...
    ui->beansAtStartup->setVisible(false);
    ui->verticalLayout_Main->removeWidget(ui->beansAtStartup);
    ...
    ui->beansAtStartup->setToolTip(ui->beansAtStartup->toolTip().arg(PACKAGE_NAME));
    ui->beansAtStartup->setText(ui->beansAtStartup->text().arg(PACKAGE_NAME));

    ui->openBeansConfButton->setToolTip(ui->openBeansConfButton->toolTip().arg(PACKAGE_NAME));
    ...
    ui->unit->setModel(new BeansUnits(this));
...
{
    /* Main */
    mapper->addMapping(ui->beansAtStartup, OptionsModel::StartAtStartup);
...
void OptionsDialog::on_openBeansConfButton_clicked()
{
