...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<BeansAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_beans-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_beans-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_beans-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = BeansUnits::formatWithUnit(unit, balance, false, BeansUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    BeansAmountField* amountInput = receiveCoinsDialog.findChild<BeansAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_beans-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
